...
def get_prompt():
    ...
    prompt_start = open('include/prompt/prompt_start.in', 'r').read()
    prompt_examples = ""
    # 读取 prompt/examples 文件夹下 exp1.in, exp5.in
    for i in [1, 22]:
        prompt_examples += open(f'include/prompt/examples/exp{i}.in', 'r').read()
    prompt_end = open('include/prompt/prompt_end.in', 'r').read()
...
def from_text_to_general_template_using_llm(text, input_length=16385, answer_length=4096):
        openai.api_key = open('include/openai_key.in', 'r').read()
        prompt = get_prompt()
        # 给 text 做截断处理
